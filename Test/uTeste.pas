...
type
  TForm1 = class(TForm)
    ...
    XDBGrid15: TXDBGrid;
    dGarantia: TDataSource;
    TabSheet17: TTabSheet;
    XDBGrid16: TXDBGrid;
    dPrestamista: TDataSource;
    GroupBox8: TGroupBox;
    Label1: TLabel;
    lGarantia: TLabel;
    GroupBox9: TGroupBox;
...
begin
  ...
  try
    lSolicitacaoDescontoModel.objeto.WhereView := ' and solicitacao_desconto.tabela_origem = ''WEB_PEDIDO'' ';
      //Aki o dataset ser� zerado apos sair da fun��o.
...
begin
  ...
  try
    lParamentros.TipoData   := 'E';
    lParamentros.DataInicio := '01/09/2024';
    lParamentros.DataFim    := '25/09/2024';
    ...
    tGarantia     := lResultado.fdGarantia;
    tPrestamista  := lResultado.fdPrestamista;
    ...
    dGarantia.DataSet     := tGarantia.objeto;
    dPrestamista.DataSet  := tPrestamista.objeto;
    ...
    lGarantia.Caption     := FormatCurr('#,###0.00',lResultado.totalGarantia);
    lPrestamista.Caption  := FormatCurr('#,###0.00',lResultado.totalPrestamista);
...
begin
  ...
  try
    try
      ID := InputBox('TRANSPORTADORA', 'Digite o c�digo da transportadora que deseja Alterar:', '');
      if ID.IsEmpty then
        exit;

      lTransportadoraModel := lTransportadoraModel.objeto.Alterar(ID);
      lTransportadoraModel.objeto.FANTASIA_TRA := 'TESTE ALTERA��O';
...
begin
  ...
  try
    try
      codigo := InputBox('TRANSPORTADORA', 'Digite o c�digo da transportadora que deseja excluir:', '');
...
begin
  ...
  try
    try
      ...
      lTransportadoraModel.objeto.RAZAO_TRA    := 'RAZ�O TRANSPORTADORA TESTE';
...
begin
  ...
  try
    try
      ID := InputBox('PrevisaoPedidoCompra', 'Digite o c�digo do PrevisaoPedidoCompra que deseja Alterar:', '');
      if ID.IsEmpty then
        exit;
...
begin
  ...
  try
    try
      ...
      Codigo_For := InputBox('PrevisaoPedidoCompra', 'Digite o C�digo Fornecedor da PrevisaoPedidoCompra que deseja excluir:', '');
...
begin
  ...
  try
    try
      ID := InputBox('Saidas', 'Digite o c�digo da Saida que deseja Alterar:', '');
      if ID.IsEmpty then
        exit;
...
begin
  ...
  try
    ...
    lPortador := InputBox('TabelaJurosDia', 'Digite o c�digo do portador:', '');
