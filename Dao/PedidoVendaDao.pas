...
begin
  ...
  try
    ...
    begin
      ...
      FPedidoVendasLista[i].SEGURO_PRESTAMISTA_CUSTO  := lQry.FieldByName('SEGURO_PRESTAMISTA_CUSTO').AsString;
      FPedidoVendasLista[i].SEGURO_PRESTAMISTA_VALOR  := lQry.FieldByName('SEGURO_PRESTAMISTA_VALOR').AsString;
...
begin
  ...
  try
    ...
    begin
      lSQL := 'SELECT BARRAS_PRO FROM PRODUTO P WHERE P.USAR_BALANCA = ''S'' AND P.CODIGO_PRO = '+ QuotedStr(((Copy(pBarrasProduto, StrToInt(lConfiguracoes.valorTag('tagConfig_BALANCA_COPY_INI_PRODUTO', '2', tvString)), StrToInt(lConfiguracoes.valorTag('tagConfig_BALANCA_COPY_FIM_PRODUTO', '6', tvString))))));
      lQry.Open(lSQL);
    end
    else if (Length(pBarrasProduto) = 13) and (Copy(pBarrasProduto, 1, 1) = '2') then
    begin
      lSQL := 'SELECT BARRAS_PRO FROM PRODUTO P WHERE P.USAR_BALANCA = ''S'' AND P.BARRAS_PRO = '+ QuotedStr(IntToStr(StrToInt(Copy(pBarrasProduto,StrToInt(lConfiguracoes.valorTag('tagConfig_BALANCA_COPY_INI_PRODUTO', '2', tvString)), StrToInt(lConfiguracoes.valorTag('tagConfig_BALANCA_COPY_FIM_PRODUTO', '6', tvString))))));
      lQry.Open(lSQL);
    end;
