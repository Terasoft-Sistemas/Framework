...
//      else Options := [TIndexOption.ixDescending, TIndexOption.ixCaseInsensitive];
    ...
    lLojasModel.objeto.LojaView := pPCG_Parametros.Lojas;
    ...
    begin
      ...
      begin
        lMemTable.InsertRecord([
                                ...
                                lQry.FieldByName('CODIGO').AsString,
...
//      else Options := [TIndexOption.ixDescending, TIndexOption.ixCaseInsensitive];
    ...
    begin
      ...
      begin
        ...
        if lQry.FieldByName('VALOR_POSSIVEL').AsFloat <> 0 then
        begin
          lValorPossivel := lQry.FieldByName('VALOR_POSSIVEL').AsFloat;
          lPercentual    := ((lQry.FieldByName('VALOR_LIQUIDO').AsFloat / lValorPossivel)*100)-100
        end;

        if pPCG_Parametros.TipoAnalise = tpFilial then
        begin
          lCodigo    := lLojas_Dados.objeto.LOJA;
          lDescricao := lLojas_Dados.objeto.DESCRICAO;
        end
        else
        begin
          lCodigo    := lQry.FieldByName('CODIGO').AsString;
          lDescricao := Trim(lQry.FieldByName('DESCRICAO').AsString);
        end;
        ...
        lMemTable.InsertRecord([
                                ...
                                lQry.FieldByName('VALOR_LIQUIDO').AsFloat,
                                lQry.FieldByName('ACRESCIMO').AsFloat,
                                lQry.FieldByName('VALOR_TOTAL').AsFloat,

                                lQry.FieldByName('QUANTIDADE_VENDA').AsFloat,
                                lQry.FieldByName('TICKET_MEDIO').AsFloat,

                                lQry.FieldByName('TOTAL_ITENS').AsFloat,
                                lQry.FieldByName('QUANTIDADE_MEDIA_ITENS').AsFloat,
                                lQry.FieldByName('VALOR_MEDIA_ITENS').AsFloat,

                                lValorPossivel,
                                lQry.FieldByName('DIFERENCA').AsFloat,
                                lPercentual,
    ...
    begin
      ...
      begin
        lMemTable.InsertRecord([
                                ...
                                lQry.FieldByName('SALDO').AsFloat
                               ]);
